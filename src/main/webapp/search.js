     ...
     function getProviderName(sstring){
        if(pname[sstring]===undefined)
	     ...
	     function removeNameSpace(sstring){
                if(sstring === "any")
			return "any";
                return sstring.substr(sstring.toString().lastIndexOf("_")+1,sstring.toString().length)
	     }
...
function parse(qs, sep, eq) {
  sep = sep || "&";
  eq = eq || "=";
  for (var obj = {},i = 0,pieces = qs.split(sep),l = pieces.length, tuple;
    i < l;
    i ++
  ) {
...
YUI({gallery: 'gallery-2011.01.03-18-30'}).use('gallery-yui3treeview', 'history','tabview','node', 'datatable-scroll', 'event', 'datasource', 'querystring-parse-simple','autocomplete', 'autocomplete-filters', 'autocomplete-highlighters', function(Y) {
		 ...
		 for (field in fields) {
		     frag = frag + "<option value='" + fields[field] + "'>" + removeNameSpace(fields[field]) + "</option>";
		 }
		 frag = frag + '</select></div>';
		 Y.one('#params').append(Y.Node.create(frag)); 
	     };

             //A value (URI or Literal) to be rendered. URI optionally has anchortext
             function renderValue(value, anchor) {

                 if (value.startsWith("urn:uuid:")) {
                     var label = value.substring(9);
                     if (anchor != undefined && anchor != null) label = anchor; 
             ...
             function getMarker(result) {
                 ...
                 return ({"id": result.title, "lat": lat, "lon" : lon});
             }
		 ...
		 if (title == null) title = result.title;
		 //Sort the quads

                 function predSort(a,b) {
		     return (fields.indexOf(encode(a.triple.predicate)) - fields.indexOf(encode(b.triple.predicate)));
                 }
		 ...
		 var table = Y.Node.create('<table class=\"result\"><col class=\"predicate-col\"/><col class=\"value-col\"/><th class="result-header">Property</th><th class=\"result-header\">Value</th></table>');
		 var i = 0;
		 for (pred in map) {
		     ...
		     row = row + "<td class=\"predicate\">" + getLocalName(pred) + "</td>" +
			 "<td>"; 
		     for (qid in map[pred]) {
                        if(labels[map[pred][qid].triple.object] != null) {
                           row = row + '<span title="'+ getProviderName(map[pred][qid].source[0])+'" class="source-' + getProviderName(map[pred][qid].source[0]) + '">'+renderValue(map[pred][qid].triple.object, labels[map[pred][qid].triple.object]) + '</span><br/>';
                        } else {
                           row = row + '<span title="'+ getProviderName(map[pred][qid].source[0])+'" class="source-' + getProviderName(map[pred][qid].source[0]) + '">'+renderValue(map[pred][qid].triple.object) + '</span><br/>';
                        }
		     }
		     row = row + '</td></tr>';
		     table.append(row);
		...
		//FIXME need handling for UUID and RDFTYPES (currently I assume all searches are on WOO attributes)
		
		var query = Y.one("#ac-input").get('value');
		
		var re2 = /(\w+:)/g;
		query = query.replace(re2,''+ns+'$1');
		query = query.replace('http_woo_corp_yahoo_com_1_7_1_ns_type:','http_www_w3_org_1999_02_22_rdf_syntax_ns_type:');
		...
		loadResults("yq=" + query);
		 ...
		 var params = Y.all(".param").each( function(thisNode, index) {
		     query = query + ' ';
		     field = thisNode.one(".field").get('value');
                     if (!Y.one("#match-mode").get('checked') && index > 0) 
                         query = query + 'OR';
		     if (field != "any")  
			 query = query + field + ':';
		     query = query + '(' + thisNode.one(".query").get('value') + ')';
		 ...
		 myDataSource.sendRequest({
			   request: query + '&m=' + Y.one("#numresults").get('value') + "&deref=" + Y.one("#dereference").get('checked'),
			   callback: {
				 ...
				 var markers = [];
				     ...
				     renderResult(results[result], ol);
                                     ...
                                     if (marker != null) {
                                        marker["label"] = result; //label the marker with index
                                        markers.push(marker);
                                     }
				 }
				 ...
				 Y.one("#results").setContent("").append(ol);
 				 Y.one("#result-stats").setContent(
					"Found " + e.response.results[0].numResults + 
					" results in " + e.response.results[0].time + " ms.");

                                 //render map if anything has coordinates
                                 if(markers.length > 0){
                                    showMap('map',markers);
				    Y.one("#result-map-tab").show();
		                 } else {
				    //TODO: fix me... this doesn't hide the tab
                                    Y.one("#result-map-tab").setContent("");
                                 }
		...
		for (i in stats.classes[clazz].properties) {
		   frag = frag + '<tr><td>' + getLocalName(stats.classes[clazz].properties[i]) + '</td><td><input class="class-property yui3-hastooltip" type="text" title="' + /* TODO */ '" name="' + getLocalName(stats.classes[clazz].properties[i]) + '"/></td></tr>';
		}
		frag = frag + '</table>';
		Y.one('#class-properties').append(Y.Node.create(frag)).show();
		 ...
		 myDataSource = new Y.DataSource.Get({
			 source: webapp + "query.do?index=" + dataset + "&format=json&"
		     });
		 ...
		 myDataSource.sendRequest({
			 ...
			 callback: {
				 ...
				 for(field in stats.fields){
				   fields.push(stats.fields[field]);
			           // fill up the autocomplete suggestion box
				   autocompletefields.push(removeNameSpace(stats.fields[field]));
				 }
				 ...
				 keys.sort();
		                 ...
		                 changeProperties(keys[0]);
              		         fragNode.on('change', function (e) { changeProperties(e.target.get('value'));});
					...
					if (stats.classes[clazz] != undefined) {
						...
						sub.push(addClass(child));
                                          ...
                                          //Class appears in the ontology but no instances  
                                          return [];
                                        }
                                        if (sub.length > 0) {
                                 	  return {label : getLocalName(clazz) + " (" + count + ") ", type : "TreeView", children: sub};
					} else {
                                 	  return {label : getLocalName(clazz) + " (" + count + ") "};
                                        }
                                 }
				 ...
				 treeview.render();
                                 ...
                                 Y.one('#statistics-tree .yui3-tree-collapsed').toggleClass('yui3-tree-collapsed');
                                 ...
                                 var sidestats = [];a
                                 for (clazz in keys) {
                                    sidestats.push({"Entity" : getLocalName(keys[clazz]), "Count": stats.classes[keys[clazz]].count});
                                 } 
				 ...
				 //On load, check if there were any query parameters
                                   if (args['q'] != undefined && args['q'] != null) {
				     var queryNode = Y.one(".query");
				     queryNode.set('value', args['q']);
				     executeSearchByField();
				   } else if (args['subject'] != undefined && args['subject'] != null ) {
        ...
        function showMap(mapid, markers) {
                ...
                if(mapid == undefined || markers == undefined || markers.length == 0 || markers[0] == undefined)
	           return;
                var myOptions = {
                                center : [parseFloat(markers[0].lat), parseFloat(markers[0].lon)],
                                mapTypeId : 0,
	                        zoom : 4
                };
	        ...
	        var components = [new ovi.mapsapi.search.component.RightClick(), new ovi.mapsapi.map.component.Behavior(), new ovi.mapsapi.map.component.TypeSelector(), new ovi.mapsapi.map.component.ZoomBar(), new ovi.mapsapi.map.component.ScaleBar(), new ovi.mapsapi.map.component.Overview(), new ovi.mapsapi.map.component.ViewControl(), new ovi.mapsapi.map.component.RightClick(), bubbleContainer];
                ...
                for(var i = 0; i < markers.length; i++){
                   //alert(markers[i].label +" " + markers[i].id + " " + markers[i].lat + markers[i].lon);
                   var gp = new ovi.mapsapi.geo.Coordinate(parseFloat(markers[i].lat), parseFloat(markers[i].lon));
                   var marker = new ovi.mapsapi.map.StandardMarker(gp, {
                      text : markers[i].label
                   });

                   marker.addListener("mouseover", function() {
                      titleBubble[i] = bubbleContainer.addBubble(markers[i].label, gp);
                   });
             ...
             //Everything below is run automatically upon loading the page

             //See if the dataset (KB name) was provided as a query parameter
               if (args['kb'] != undefined && args['kb'] != null) {
		  Y.one("#dataset").append('<option value=\"' + args['kb'] + '\">' + args['kb'] + '</option>');
	     ...
	     Y.one('#search-by-field').on('submit', executeSearchByField);
	     ...
	     //On submit, retrieve and display search results
             ...
             var tabview = new Y.TabView({srcNode: '#resultContainer'});
             tabview.render();
