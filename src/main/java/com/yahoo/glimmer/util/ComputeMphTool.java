...
public class ComputeMphTool extends Configured implements Tool {
	public static final FsPermission ALL_PERMISSIONS = new FsPermission(
	...
	public long buildMph(final FileSystem fs, final String srcFilename) throws IOException {
		...
		LcpMonotoneMinimalPerfectHashFunction<CharSequence> mph = new LcpMonotoneMinimalPerfectHashFunction<CharSequence>(inCollection, TransformationStrategies.prefixFreeUtf16());
	...
	/**
	 * Presents a Reader as a Collection of MutableStrings with each line read from the Reader as an element of the Collection.
	 * {@link LineReaderCollection.ReaderFactory.newReader} is called each time {@link LineReaderCollection.iterator} is called.
	 * Only the current line and next line are held in memory.
	 * 
	 * Note that {@link LineReaderCollection.LineReaderIterator.next} always returns the same instance of
	 * MutableString(but with different contents) for each instance
	 * of {@link LineReaderCollection.LineReaderIterator}.
	 *  
	 * @author tep
	 *
	 */
