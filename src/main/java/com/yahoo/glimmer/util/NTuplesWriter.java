...
public class NTuplesWriter implements RDFHandler {
    ...
    // We need the URL for bNode globalization even when writing out triples
    // only
	...
	if (st instanceof ContextStatementImpl)
	    ctx = ((ContextStatementImpl) st).getContext();
	...
	if (!sha1Cache.containsKey(url.toExternalForm())) {
	    try {
		sha1Cache.put(url.toExternalForm(), Util.createSHA1(url.toExternalForm()));
