...
public class DistributedCollectionBuilder extends Configured implements Tool {
	...
	public static class MapClass extends 
			Mapper<LongWritable, Document, Text, Text> {
		...
		public void setup(Context context) {
			...
			try {
				...
				if (!fs.exists(path)) {
					fs.mkdirs(path);
					FsPermission allPermissions = new FsPermission(
		...
		//Similar to SimpleCompressedDocumentBuilder.build() but we don't have a Sequence
		//and our WordArrayReader doesn't support setReader
		@Override
		public void map(LongWritable key, Document document, Context context)
				throws IOException {
			...
			for ( int field = 0; field < numberOfFields; field++ ) {
				...
				if ( factory.fieldType( field ) == FieldType.TEXT ) {
					...
					while ( content.next( word, nonWord ) ) builder.add( word, nonWord );
	...
	private void printUsage() {
		System.out
				.println("Usage : DistributedCollectionBuilder <input_dir> <output_dir> horizontal|vertical datarss|ntuples {indexedproperties_file}");
	}
