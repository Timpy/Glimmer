...
/**
 * A document sequence obtained by breaking an input stream at a specified
 * separator.
 * 
 * <p>
 * This document sequences blindly passes to the indexer sequences of characters
 * read in a specified encoding and separated by a specified byte.
 */
...
public class DataRSSDocumentSequence extends FastBufferedInputStream implements DocumentSequence {
    ...
    /**
     * If true, the last returned stream has been exhausted. This variable will
     * be reset by a call to <code>nextDocument()</code>.
     */
    ...
    /**
     * Creates a new document sequence based on a given input stream and
     * separator; the sequence will not return more than the given number of
     * documents.
     * 
     * @param inputStream
     *            the input stream containing all documents.
     * @param separator
     *            the separator.
     * @param factory
	...
	return new DocumentIterator() {
		...
		if (i >= maxDocs)
		    return last = null;
		// If eof is not true, the caller did not exhaust the current
		// document. We do it, however.
		if (!eof)
		    DataRSSDocumentSequence.this.close();
		eof = false;
	...
	if (avail <= 0) {
	    // Ooops, there's nothing more to read. Let us set up eof and
	    // return.
	    avail = 0;
	    eof = true;
	    return true;
	...
	for (;;) {
	    ...
	    // We scan the buffer for the separator, copying elements in the
	    // mean time.
	    for (i = 0; i < l; i++) {
		if (buffer[pos + i] == separator)
		    break;
