...
public class TripleIndexGenerator extends Configured implements Tool {
	...
	public void open() throws IOException {
	    ...
	    skipBitStreamIndexWriter = new SkipBitStreamIndexWriter(index, offsets, posNumBits, numDocs, TEMP_BUFFER_SIZE, defaultStandardIndex, QUANTUM,
		    HEIGHT);
	...
	public void close(long writtenOccurrences) throws IOException {
		...
		System.out.println("Closing index " + indexName + " which has " + props.getProperty(it.unimi.dsi.mg4j.index.Index.PropertyKeys.TERMS)
			+ " terms ");
		if (positions) {
		 ...
		 */
		props.setProperty(it.unimi.dsi.mg4j.index.Index.PropertyKeys.TERMPROCESSOR, RDFDocumentFactory.TERMPROCESSOR);
    ...
    public static class Occurrence implements WritableComparable<Occurrence>, Cloneable {
     ...
     * 
     */
    public static class TermOccurrencePair implements WritableComparable<TermOccurrencePair> {
	...
	public String toString() {
	    return "(" + index + "," + term + "," + occ.document + "," + occ.position + ")";
	}
	...
	WritableComparator.define(TermOccurrencePair.class, new TermOccurrencePairComparator());
    }
	...
	try {
	    ...
	    if (context != null) {
		defaultMetadata.put(RDFDocumentFactory.MetadataKeys.MAPPER_CONTEXT, context);
	    }
	...
	public void map(LongWritable key, Document doc, Context context) throws IOException, InterruptedException {
		...
		while (termReader.next(term, nonWord)) {
			...
			if (factory instanceof VerticalDocumentFactory) {
				...
				Occurrence predicateOcc = new Occurrence(predicateID, -1);
				context.write(new TermOccurrencePair(term.toString(), ALIGNMENT_INDEX, predicateOcc), predicateOcc);
				Occurrence fakePredicateOccurrrence = new Occurrence(-1, predicateID);
    ...
    public static class ReduceClass extends Reducer<TermOccurrencePair, Occurrence, Text, Text> {
	...
	public void setup(Context context) {
		...
		Class<?> documentFactoryClass = job.getClass(RDFInputFormat.DOCUMENTFACTORY_CLASS, RDFDocumentFactory.class);
		DocumentFactory factory = initFactory(documentFactoryClass, job, null, false);
		...
		if (factory instanceof VerticalDocumentFactory) {
		    ...
		    Index index = new Index(fs, outputDir + uuid, ALIGNMENT_INDEX_NAME, job.getInt(NUMBER_OF_DOCUMENTS, -1), false);
		    index.open();
		...
		for (int i = 0; i < factory.numberOfFields(); i++) {
			...
			System.out.println("Opening index for field:" + name + " Heap size: current/max/free: " + heapSize + "/" + heapMaxSize + "/"
				+ heapFreeSize);

			Index index = new Index(fs, outputDir + uuid, name, job.getInt(NUMBER_OF_DOCUMENTS, -1), true);
			index.open();
	...
	public void reduce(TermOccurrencePair key, Iterable<Occurrence> values, Context context) throws IOException {
	    ...
	    currentIndex.getIndexWriter().writeFrequency(Math.min(numDocs, MAX_INVERTEDLIST_SIZE));
		...
		if (docID != prevDocID) {
			...
			System.err.println("More than " + MAX_INVERTEDLIST_SIZE + " documents for term " + key.term);
			break;
		    }
		    posIndex = 0;
			...
			System.err.println("More than " + MAX_POSITIONLIST_SIZE + " positions for term " + key.term);
		    } else {
			buf[posIndex++] = occ.position;
		    }
		}
	...
	job.getConfiguration().setClass("mapred.output.key.comparator.class", TermOccurrencePairComparator.class, WritableComparator.class);
	...
	if (args.getString("properties") != null) {
	    job.getConfiguration().set(INDEXEDPROPERTIES_FILENAME_KEY, args.getString("properties"));
	}
