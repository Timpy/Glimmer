...
public class WordIndexGenerator extends Configured implements Tool {
	...
	public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
	    ...
	    FastBufferedReader fbr = new FastBufferedReader(new StringReader(value.toString()));
		...
		if (word != null && !word.equals("")) {
		    ...
		    output.collect(new Text(word.toString()), new IntWritable(Integer.parseInt(parts[0])));
	...
	public void configure(JobConf job) {
		...
		Path indexPath = new Path(outputDir + uuid + "/" + INDEX_NAME + DiskBasedIndex.INDEX_EXTENSION);
		index = new OutputBitStream(fs.create(indexPath, true));// overwrite
		fs.setPermission(indexPath, allPermissions);

		Path termsPath = new Path(outputDir + uuid + "/" + INDEX_NAME + DiskBasedIndex.TERMS_EXTENSION);
		...
		Path offsetsPath = new Path(outputDir + uuid + "/" + INDEX_NAME + DiskBasedIndex.OFFSETS_EXTENSION);
		...
		skipBitStreamIndexWriter = new SkipBitStreamIndexWriter(index, offsets, null, job.getInt(NUMBER_OF_DOCUMENTS, -1), 1024 * 1024 * 32,
			defaultStandardIndex, 16, 10);
	...
	public void reduce(Text key, Iterator<IntWritable> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
		...
		if (count++ > MAX_POSTING_LIST_SIZE) {
		    System.err.println("Posting list is too big for key:" + key.toString());
		    return;
		}
	    ...
	    for (int i = 0; i < documentIDs.size(); i++) {
		OutputBitStream out = skipBitStreamIndexWriter.newDocumentRecord();
		skipBitStreamIndexWriter.writeDocumentPointer(out, documentIDs.get(i));
	    }
	...
	public void close() throws IOException {
		...
		props.setProperty("termprocessor", "it.unimi.dsi.mg4j.index.NullTermProcessor");
