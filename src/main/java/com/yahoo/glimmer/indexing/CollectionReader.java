...
public class CollectionReader {
	...
	// final SimpleCompressedDocumentCollection test =
	// (SimpleCompressedDocumentCollection) AbstractDocumentSequence.load(
	// "/mnt/scratch2/pmika/data/btc/collection/small/triples-04993e8d-7d5b-4511-b85a-cc5db85338ac.collection");
	...
	for (int i = 0; i < collectionNames.length; i++) {
	    ...
	    // collectionNames[i]).getParentFile();
	    try {
		// documentCollection = (SimpleCompressedDocumentCollection)
		// AbstractDocumentSequence.load(collectionString);
	...
	/*
	 * final ConcatenatedDocumentCollection collection = new
	 * ConcatenatedDocumentCollection(collectionNames);
	 * 
	 * String filename = args[0] + collectionNames[0];
	 * 
	 * File parent = filename != null ? new File( filename.toString()
	 * ).getParentFile() : null; System.err.println(new File( parent,
	 * collectionNames[ 0 ] ).toString() );
	 * 
	 * collection.filename(args[0] + collectionNames[0]);
	 * //ConcatenatedDocumentCollection will take the parent of this
	 */
	if (args.length > 2) {
