 ...
 * which may contain either short names of the form prefix_localname, e.g. fn_vcard
 * or full URIs. In case of short names, the URI of the predicate should be possible to
 * convert to a shortname using the namespaces table. In case of URIs, the URI in the file
 * should be convertible using the namespaces table.
 * 
...
public class VerticalDocumentFactory extends RDFDocumentFactory {
	...
	protected void init() {
		super.init();
		//Retrieve properties to index from metadata, or if fails from the filesystem or jar
			...
			try {
				...
				while ((nextLine = reader.readLine())!= null) {
					if (!nextLine.trim().equals("")) {
						...
						String property = nextLine.split("\\s")[0].trim(); //if no match, returns the whole string
						
						//Only include if it's in the namespaces table and not blacklisted
		...
		private void ensureParsed() throws IOException {
			...
			try {
				...
				for (Statement stmt: handler.getStatements()) {
					...
					/*
					if (fieldName == null) {
						System.err.println("Could not convert URI to field name: " + predicate);
						if (reporter != null) reporter.incrCounter(Counters.CANNOT_CREATE_FIELDNAME_TRIPLES, 1);
						//Not in table, don't index
						continue;
					}
					*/
	    			...
	    			if (fieldIndex == -1) {
	    				...
	    				if (mapContext != null) mapContext.getCounter(TripleIndexGenerator.Counters.UNINDEXED_PREDICATE_TRIPLES).increment(1);
	    				continue; 
	    			}
	
	    			if (stmt.getObject() instanceof Resource) {
	    				//Encode the resource URI or bnode ID using the MPH for objects
		    			...
		    			while (fbr.next(word, nonWord)) {
		        			if (word != null && !word.equals("")) {
		    					...
		    					if (TERMPROCESSOR.processTerm(word)) {
			        				...
			        				//System.out.println("Adding to field " + fieldIndex + " " + word.toString());
		        				}
		        			}
		        		}
		    			fbr.close();
	...
	public final static void main(String[] args) {
		String result = null;
		String predicate = "http://creativecommons.org/ns#attributionName";
